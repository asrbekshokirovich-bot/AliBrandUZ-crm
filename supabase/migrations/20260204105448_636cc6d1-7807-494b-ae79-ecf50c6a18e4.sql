-- Update confirm_arrived_products function with:
-- 1. Weighted average cost calculation for variants
-- 2. Auto-update box status when all items confirmed
...
BEGIN
  ...
  -- Auto-update box status when all items confirmed
  UPDATE boxes b
  SET 
    status = 'arrived',
    location = 'uzbekistan',
  ...
  AND NOT EXISTS (
    ...
    AND pi2.status NOT IN ('in_tashkent', 'sold', 'returned')
  );
