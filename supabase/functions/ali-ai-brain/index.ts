...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { message, conversationId, stream: enableStream, action, imageBase64, mimeType } = body;

    // ── NEW: Return Document Analysis ────────────────────────────────────────
    if (action === 'analyze_return_doc') {
      if (!imageBase64) {
        return new Response(
      ...
      const clarityRes = await fetch(AI_GATEWAY, {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: [
            { type: 'text', text: 'Bu hujjat tasvirini baholasang. Faqat JSON formatda javob ber: {"clarity_score": 0.9, "is_readable": true, "clarity_issues": ""}. Agar clarity_score 0.6 dan past bolsa is_readable false qil va muammoni yoz.' },
            imageContent,
          ]}],
      ...
      if (clarityRes.ok) {
        const cd = await clarityRes.json();
        const cc = cd.choices?.[0]?.message?.content || '{}';
        try { const m = cc.match(/\{[\s\S]*\}/); if (m) clarityResult = JSON.parse(m[0]); } catch {}
      }

      if (!clarityResult.is_readable || clarityResult.clarity_score < 0.55) {
        return new Response(JSON.stringify({
          success: true, is_readable: false,
          clarity_score: clarityResult.clarity_score,
          clarity_issues: clarityResult.clarity_issues || "Hujjat xira yoki o'qish qiyin",
          items: [], document_info: null,
      ...
      const extractRes = await fetch(AI_GATEWAY, {
        ...
        body: JSON.stringify({
          model: AI_MODEL,
          messages: [{ role: 'user', content: [
            { type: 'text', text: `Bu "Vozvrat tovarlar" hujjatidan faqat JSON chiqar (boshqa matn yoq):
{
  "document_info": {"doc_number":"","date":"","client_name":"","client_phone":"","contract_number":"","commissioner":""},
  "items": [{"num":1,"sku":"","name":"","barcode":null,"price":0,"qty":0,"total":0}],
  "grand_total_qty": 0,
  "grand_total_sum": 0
}` },
            imageContent,
      ...
      if (extractRes.ok) {
        ...
        try { const m = ec.match(/\{[\s\S]*\}/); if (m) extractResult = JSON.parse(m[0]); } catch {}
      }
