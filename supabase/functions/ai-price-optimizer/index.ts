...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'optimize_single') {
      ...
      const currentPrice = parseFloat(String(listing.price || 0));
      const costPrice = parseFloat(String(listing.cost_price || currentPrice * 0.6)); // Estimate if not set
      const commissionRate = parseFloat(String(listing.commission_rate || 0.15));
...
5. SABAB: [qisqa tushuntirish o'zbek tilida]`;
      ...
      if (!aiResponse.ok) {
        ...
        const recommendedPrice = priceMatch 
          ...
          : parseFloat(avgCompetitorPrice.toFixed(2));
