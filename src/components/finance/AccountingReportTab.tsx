    ...
    try {
      ...
      if (allProductIds.length > 0) {
        const products = await fetchAllRows(
          supabase.from('products')
            .select('id, shipping_cost_to_china, quantity, cost_price, purchase_currency')
            .in('id', allProductIds)
        );
        for (const p of products) {
          productsMap[p.id] = {
            ...
            purchaseCurrency: (p as any).purchase_currency || 'CNY',
          };
        }
      }
      ...
      const rows: ReportRow[] = Object.entries(skuMap).map(([sku, data]) => {
        ...
        if (variantCost > 0) {
          ...
          const cur = product.purchaseCurrency;
          if (cur === 'CNY') chinaPrice = product.costPrice * cnyToUzs;
          else if (cur === 'USD') chinaPrice = product.costPrice * usdToUzs;
          else chinaPrice = product.costPrice; // UZS
        } else {
